...
// --- Reusable Feature Card Component with Neon Box Shadow ---
...
const LandingPage = () => {
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#091C2D] text-white flex flex-col font-poppins overflow-hidden">
      ...
      {/* Footer section that does not grow, sits at the bottom */}
      <div className="relative z-10 w-full flex-shrink-0 pb-8">
        {/* --- MOBILE FOOTER --- */}
        <footer className="md:hidden w-full max-w-md mx-auto text-center space-y-4 px-6">
          <Button onClick={() => router.push('/sign-up')} className="w-full bg-[#34D399] text-[#0D1117] font-bold py-3 rounded-lg text-lg hover:bg-[#2cb985]">
            Sign Up
          </Button>
          <Button onClick={() => router.push('/sign-in')} variant="outline" className="w-full border-2 border-[#34D399] text-white font-bold py-3 rounded-lg text-lg hover:bg-[#34D399] hover:text-[#0D1117]">
            Log In
          </Button>
        </footer>
        ...
        <footer className="hidden md:block text-center">
            <Button 
              ...
              className="bg-[#34D399] text-[#0D1117] font-bold rounded-lg text-lg h-14 px-10 hover:bg-[#2cb985] transition-all duration-300 transform hover:scale-105"
            >
              Get Started
            </Button>
        </footer>
